--[[
    ...
    Notes:
        WAR gets shafted for Club and Staff options. More viable to use PLD for skillups
        H2H is better saved for MNK
]]
local profile = {};
...
local sets = {
    ...
    RA_Base      = {
        Ring1   = 'Woodsman\'s ring',
        Ring2   = 'Woodsman\'s ring',
    },
};
profile.Sets = sets;
...
-- RA Sets are in referse as the Base is RAcc gear and the Int application would be steadily lowering acc for int.
-- Magic Accuracy should apply to bolts if magic affinity works for them.

-- Note. /BLM added +2 int on top of the Int_Mid set and results were still shit.
sets.RA_Int_High    = gFunc.Combine(sets.RA_Base, {
        --Head    = '',
        --Body    = '',
        --Hands   = 'Garrison Gloves,       -- +1
        --Back    = '',           -- Can't equip Black Cape
        --Waist   = '',
        --Legs    = '',
});
sets.RA_Int_Mid     = gFunc.Combine(sets.RA_Int_High, {
        Head    = 'Erd. Headband',          -- +1
...
sets.RA_Int_Low     = gFunc.Combine(sets.RA_Int_Mid, {});
...
profile.OnLoad = function()
    ...
    AshitaCore:GetChatManager():QueueCommand(-1, '/alias /bolt /lac fwd Bolt');
end
...
profile.OnUnload = function()
    ...
    AshitaCore:GetChatManager():QueueCommand(-1, '/alias delete /bolt');
end
...
profile.HandleCommand = function(args)
    ...
    elseif(args[1] == 'Bolt') then
        Settings.Bolt = not Settings.Bolt;
        if(Settings.Bolt) then
            gFunc.Message("Enabling INT for Bolts.");
        else
            gFunc.Message("Disabling INT for Bolts");
        end
...
profile.HandleMidshot = function()
    if(Settings.Bolt) then
        gFunc.EquipSet('RA_Int_' .. TPModeTable[Settings.TP_Mode]);
    else
        gFunc.EquipSet(sets.RA_Base);
    end
end
